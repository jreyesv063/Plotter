...
class Limits:
    ...
    def datacards_creation(self, bgr_case, precision):
        ...
        systMaster = [
            ...
            # MET
            [f"CMS_scale_met_unclustered_energy_{bgr_case}_{self.channel}_{self.year}", "shape"], #
            # MET trigger 
            ...
            [f"pdf_lha_alphas_{bgr_case}_{self.channel}_{self.year}", "shape"], #
                    ...
                    for j, (syst_name, syst_type) in enumerate(systMaster):
                        ...
                        for i, proc_name in enumerate(allNames):
                            ...
                            if i == 0:  # Señal
                                ...
                                # Verificación de errores para Señal
                                if hist_up is None or hist_down is None or rate is None:
                                    raise RuntimeError(f"ERROR: Histograma Up/Down no encontrado para SIGNAL con sistemático: {syst_name}")
                                ...
                                # Verificación de errores para Background
                                if hist_up is None or hist_down is None or rate is None:
                                    raise RuntimeError(f"ERROR: Histograma Up/Down no encontrado para el proceso '{proc_name}' con sistemático: {syst_name}")


                            #print(syst_name, proc_name, hist_up, hist_down, rate)
                            ...
                            if hist_up and hist_down and rate > 0:
                                ...
                                val_up = "-" if abs(up_val - 1) == 0 else 1
                                val_down = "-" if abs(down_val - 1) == 0 else 1
    ...
    for mass in {mass_list}; do
      combineCards.py \\
        signal="{input_dir}/wprime_signal_tau_{year}_M_${{mass}}.txt" \\
        tt="{input_dir}/wprime_tt_tau_{year}_M_${{mass}}.txt" \\
        wj="{input_dir}/wprime_wj_tau_{year}_M_${{mass}}.txt" \\
