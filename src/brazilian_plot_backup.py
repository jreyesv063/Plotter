...
class BrazilianPlot:
    ...
    def main(self, runBlinded, grid):
       ...
       # --------------------------------------
        ...
        # Recolectamos TODOS los valores (límites y teoría) para encontrar el máximo real
        all_values = []
        for limits in limitNumbers:
            all_values.extend(limits)
        all_values.extend(theory) # IMPORTANTE: Incluir la curva roja en el cálculo
        
        # Definimos el máximo con un margen para la leyenda
        # En escala logarítmica, un factor de 10-100 es mejor que un 1.3
        y_max = max(all_values) * 50.0 
        
        # En escala logarítmica el mínimo DEBE ser > 0
        y_min = min(all_values) * 0.1
        if y_min <= 0: y_min = 0.001 # Valor de seguridad

        # Create a frame histogram to define axes
        # Usamos un nombre único para evitar conflictos en memoria de ROOT
        frame_hist = ROOT.TH1F("frame_hist_" + str(self.year), "", 100, x_min, x_max)
        ...
        frame_hist.GetXaxis().SetTitle("m_{W'} (GeV)")
        frame_hist.GetYaxis().SetTitle("95% CL upper limit on #sigma #times BR (pb)")
        
        # Ajustes de tamaño y offsets
        frame_hist.GetXaxis().SetTitleSize(0.05)
        frame_hist.GetYaxis().SetTitleSize(0.05)
